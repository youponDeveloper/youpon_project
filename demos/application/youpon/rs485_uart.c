...
extern void rs485_request_subdevice_state();
...
//55 AA 55   000F  01  00   FFFF  0000   03  01 0008  00  0011  FF
void rs485_request_lightwarm_level(void)
{
	...
	ctrlparam.destinationAddress = 0xFFFF;
	ctrlparam.srcAddress = RS485_CTRL_ATTR_ID;
	ctrlparam.destEndpoit = USER_NUMBER_VAL_3;//lightwarm
	ctrlparam.srcEndpoit = USER_NUMBER_VAL_1;
	ctrlparam.pileId = 0x0008;
	ctrlparam.cmdId = USER_NUMBER_VAL_0;
	ctrlparam.cmd_length = USER_NUMBER_VAL_0;
	ctrlparam.cmd[ctrlparam.cmd_length++]=0x00;
	ctrlparam.cmd[ctrlparam.cmd_length++]=0x11;
	Rs485_SendDataPackageEx(&ctrlparam);
}


int rs485_date_sendto_parse(uint8_t *sendbuff)
{
	cJSON *root;
...
void rs485_uart_data_recv(mico_thread_arg_t arg )
{
	...
	rs485_request_lightwarm_level();
	rs485_request_subdevice_state();
	while(1)
	{			
		...
		{		
			...
			for (i = 0; i < recv_len; i++)
			{
				...
				#endif
			rs485_date_sendto_parse(sendbuff);
...
int rs485_RequestOneNotIRSensorState()
{
	static int idx = 0;
	int ret = -1;
...
{	
	...
	if(count%2==0)//IR sensor use 1.5s for stable test
