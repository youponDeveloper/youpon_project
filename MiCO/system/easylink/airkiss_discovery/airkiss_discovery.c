...
static void ak_discovery_thread( uint32_t arg )
{
  ...
  extern int XlinkSystemGetDeviceID(void);
  int deviceid = 0;
  char strdeviceid[20];
    ...
    if( FD_ISSET( udp_fd, &readfds ) )
    {
      ...
      switch (ret){
        case AIRKISS_LAN_SSDP_REQ:
          ...
          ret = airkiss_lan_pack(AIRKISS_LAN_SSDP_RESP_CMD, _appid, strdeviceid, 0, 0, buf, (unsigned short *)&len, &akconf);
