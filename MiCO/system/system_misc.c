...
extern void wifi_led_quick_glitter_start(int gpio);
...
{
  extern  void Send_wifi_status(unsigned char sta); //add by liding
  ...
  case NOTIFY_STATION_UP:
    ...
    //wifi_lighten_led_color(true, WIFI_LED_GPIO_G);
    IPStatusTypedef para;
    micoWlanGetIPStatus(&para, Station);
    system_log("ip: %s", para.ip);
    Send_wifi_status(1);//add by liding
    break;
  case NOTIFY_STATION_DOWN:
    system_log("Station down");
   // MicoRfLed(false);
    //wifi_lighten_led_color(true, WIFI_LED_GPIO_R);
    Send_wifi_status(0);//add by liding
    break;
  case NOTIFY_AP_UP:
...
void system_connect_wifi_normal( system_context_t * const inContext)
{
  ...
  wifi_led_quick_glitter_start(0);
...
OSStatus system_network_daemen_start( system_context_t * const inContext )
{
  ...
  str2hex((unsigned char *)para.mac, get_mac_buf, 6); //add by lid
  get_mac(get_mac_buf,6);
  system_log("ip: %s", para.ip);
